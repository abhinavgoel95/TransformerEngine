...
__device__ __forceinline__ nvfp4_scale_t compute_decoding_scaling_factor(const float block_amax,
  ...
  // return S_dec_b_fp8;
  // TODO(Frank): Divide by 6.0f is not elegant and not efficient.
...
__device__ __forceinline__ uint32_t get_rbits(curandStatePhilox4_32_10_t &state,
...
void nvfp4_quantize_transpose(const Tensor &input, const Tensor *noop, Tensor *output,
  ...
  // TODO(Frank): This is a temporary fix to enable stochastic rounding, since the cpp is not yet plumbed.
  // TODO(Frank): Remove this once the quant_config_cpp is plumbed.
  bool use_stochastic_rounding =
      false;  // quant_config_cpp ? quant_config_cpp->stochastic_rounding : false;
