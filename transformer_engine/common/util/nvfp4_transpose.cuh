...
constexpr size_t RNG_GENS_PER_THREAD = SCALES_PER_THREAD / 4;   // Each call generates 4x uint32_t random numbers 
...
__device__ __forceinline__ nvfp4_scale_t compute_decoding_scaling_factor(const float block_amax,
  ...
  // return S_dec_b_fp8;
  return static_cast<nvfp4_scale_t>(block_amax * rcp_6f * S_enc);
}
...
__device__ __forceinline__ fp4e2m1x4
mul_cvt_fp32_to_fp4_4x_with_stochastic_rounding(const float2 in01, const float2 in23, const float2 scale, const uint32_t rbits) {
  uint16_t out_4x = 0;
#if CUDA_ARCH_HAS_CVT_FEATURE
  asm volatile(
      "{\n"
...
get_rbits(curandStatePhilox4_32_10_t& state, uint4& random_uint4, int& rnd_idx) {
  if (rnd_idx == 4) {
    rnd_idx = 0;
    random_uint4 = curand4(&state);
  }
...
template <bool COMPUTE_ACTIVATIONS, typename ParamOP, float (*OP)(float, const ParamOP &), typename IType>
__global__ void __launch_bounds__(THREADS_NUM)
    nvfp4_transpose_kernel(const __grid_constant__ CUtensorMap tensor_map_input,
                           ...
                           const size_t rng_seed, const size_t rng_sequence) {
#if (defined __CUDA_ARCH__) && (__CUDA_ARCH__ >= 1000)
  ...
  curand_init(rng_seed, rng_sequence, rng_offest, &state);

  uint4 random_uint4 = curand4(&state);
  uint32_t* rbits_arr =  reinterpret_cast<uint32_t*>(&random_uint4); 
  int rnd_idx = 0;  // Index of the random number. It increments each time when used and resets to 0 if reaches 4x  
  ...
  const size_t SFs_per_col = rows / SCALE_DIM;
  ...
  constexpr size_t buff_size_scales = CHUNK_DIM_Y * SCALES_PER_CHUNK_X * sizeof(nvfp4_scale_t);
  constexpr size_t buff_size_scales_t = SCALES_PER_CHUNK_Y * CHUNK_DIM_X * sizeof(nvfp4_scale_t);
  ...
  nvfp4_scale_t *out_rowwise_scales_sh = reinterpret_cast<nvfp4_scale_t *>(dshmem + in_mem + out_mem_rowwise_data + out_mem_colwise_data);
  nvfp4_scale_t *out_colwise_scales_sh = reinterpret_cast<nvfp4_scale_t *>(dshmem + in_mem + out_mem_rowwise_data + out_mem_colwise_data + out_mem_rowwise_scales);
  ...
  for (size_t stage = 0; stage < STAGES; ++stage) {
    ...
    // COLWISE scaling
    {
      ...
      for (size_t it = 0; it < ITERATIONS_TRANSPOSE; ++it) {
        ...
        const size_t shmem_offset_base_colwise_in = buff_offset_in + in_thread_offset_Y * BUFF_IN_DIM_X 
                                                    + in_thread_offset_X;
        const size_t shmem_offset_base_colwise_out_t = buff_offset_out_t
        ...
        const float block_scale_inverse = __fdiv_rn(S_enc_colwise, static_cast<float>(S_dec_b_fp8));  // S_enc_b_fp8
        ...
        for (int e = 0; e < SCALE_DIM / 4; ++e) {
          if constexpr (NO_ACTIVATIONS_NOT_FP32_INPUT) {
            ...
            regs[e] = mul_cvt_bf16_to_fp4_4x_with_stochastic_rounding(elts, block_scale_inverse_2x, rbits);
          } else {
            ...
            regs[e] = mul_cvt_fp32_to_fp4_4x_with_stochastic_rounding(in01, in23, block_scale_inverse_2x, rbits);
          }
        }
        ...
        uint32_t* out_t_data_sh_as_uint32_t = reinterpret_cast<uint32_t*>(&out_t_data_sh[shmem_offset_base_colwise_out_t]);
    ...
    // ROWWISE scaling
    {
      ...
      for (size_t it = 0; it < ITERATIONS_NORMAL; ++it) {
        ...
        const size_t shmem_offset_base_rowwise_in = buff_offset_in + it_thread_offset_Y_rowwise * BUFF_IN_DIM_X;
        const size_t shmem_offset_base_rowwise_out = buff_offset_out + it_thread_offset_Y_rowwise * BUFF_OUT_DIM_X;
        ...
        // Check boundaries
        const size_t scales_offset_Y = scales_offset_Y_rowwise
                                       + stage * BUFF_DIM_Y
        ...
        const bool rowwise_scale_is_within_bounds_Y = (stage_rowwise_scales_offset_Y + it * THREADS_Y_ROWWISE + tid_Y_rowwise) < chunk_rows;
        if (rowwise_scale_is_within_bounds_X && rowwise_scale_is_within_bounds_Y) {
          scales_ptr[scale_idx_global] = S_dec_b_fp8;
        }

        // Compute "correct" per-block encoding scaling factor
        const float block_scale_inverse = __fdiv_rn(S_enc_rowwise, static_cast<float>(S_dec_b_fp8));  // S_enc_b_fp8
        const float2 block_scale_inverse_2x{block_scale_inverse, block_scale_inverse};
        ...
        for (int w = 0; w < WAVES; ++w) {
          ...
          for (int e = 0; e < PACK_SIZE / 4; ++e) {
            ...
            if constexpr (NO_ACTIVATIONS_NOT_FP32_INPUT) {
              ...
              out.data.elt[e] = mul_cvt_bf16_to_fp4_4x_with_stochastic_rounding(elts, block_scale_inverse_2x, rbits);
            } else if constexpr (IS_CACHED_ACT_OP) {
              ...
              out.data.elt[e] = mul_cvt_fp32_to_fp4_4x_with_stochastic_rounding(in01, in23, block_scale_inverse_2x, rbits);
            }
          }
          const size_t swizzled_group_idx = ((w + bank_group) * PACK_SIZE) % SCALE_DIM;
  ...
  if (colwise_scale_is_within_bounds_Y) {
    ...
    if (chunk_rows >= CHUNK_DIM_Y) {
      scales_vec.store_to(&scales_t_ptr[scale_idx_global]);
    } else {
      scales_vec.store_to_elts(&scales_t_ptr[scale_idx_global], 0, chunk_rows / SCALE_DIM);
    }
  }
...
void nvfp4_quantize_transpose(const Tensor &input, const Tensor *noop, Tensor *output,
  ...
  NVTE_CHECK(rows % 32 == 0, "Number of tensor rows must be a multiple of 32"); // 16B alignment for TMA
  NVTE_CHECK(cols % 32 == 0, "Number of tensor cols must be a multiple of 32"); // 16B alignment for TMA
  ...
  nvfp4_scale_t *const scales_transpose_ptr = reinterpret_cast<nvfp4_scale_t *>(output->columnwise_scale_inv.dptr);
  
  const size_t scale_rows_rowwise = output->scale_inv.shape[0];
  const size_t scale_cols_rowwise = output->scale_inv.shape[1];
  const size_t scale_elts_num_rowwise = scale_rows_rowwise * scale_cols_rowwise;
  const size_t scale_elts_size_total_rowwise = scale_elts_num_rowwise * sizeof(nvfp4_scale_t);
  
  const size_t scale_rows_colwise = output->columnwise_scale_inv.shape[0];
  const size_t scale_cols_colwise = output->columnwise_scale_inv.shape[1];
  const size_t scale_elts_num_colwise = scale_rows_colwise * scale_cols_colwise;
  const size_t scale_elts_size_total_colwise = scale_elts_num_colwise * sizeof(nvfp4_scale_t);
  ...
  const size_t rng_blocks_total = RNG_GENS_PER_THREAD * THREADS_NUM * grid.x * grid.y;
  ...
  cudaFuncSetAttribute(
      nvfp4_transpose_kernel<COMPUTE_ACTIVATIONS, ParamOP, OP, IType>,
