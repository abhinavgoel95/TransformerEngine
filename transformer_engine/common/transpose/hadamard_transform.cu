...
#include "common/transpose/hadamard_transform.h"
...
__device__ __forceinline__ void ReduceMax(const float pre_rht_amax, const float identity_amax,
  ...
  float warp_pre_rht_amax = kReturnPreRhtAmax ? WarpReduceMax<kWarpSize>(pre_rht_amax) : 0.0f;
  float warp_identity_amax = kReturnIdentityAmax ? WarpReduceMax<kWarpSize>(identity_amax) : 0.0f;
  float warp_transpose_amax =
      kReturnTransposedAmax ? WarpReduceMax<kWarpSize>(transpose_amax) : 0.0f;
  ...
  if (warpid == 0) {
    if (kReturnIdentityAmax) {
      ...
      identity_accum = WarpReduceMax<kNumWarpsPow2>(identity_accum);
      if (local_rank == 0) {
  ...
  if (warpid == 1) {
    if (kReturnTransposedAmax) {
      ...
      transpose_accum = WarpReduceMax<kNumWarpsPow2>(transpose_accum);
      if (local_rank == 0) {
  ...
  if (warpid == 2) {
    if (kReturnPreRhtAmax) {
      ...
      pre_rht_accum = WarpReduceMax<kNumWarpsPow2>(pre_rht_accum);
      if (local_rank == 0) {
...
__global__ void HadamardAmaxTmaKernel(const __grid_constant__ CUtensorMap tensor_map_input,
  ...
  // TODO(Frank): Align? Currently not an issue.
  uint8_t* dshmem = reinterpret_cast<uint8_t*>(base_shmem_ptr);
...
__global__ void HadamardTransformKernel(const T* __restrict__ input, T* __restrict__ output,
  ...
  if constexpr (kUpdateIdentityAmax) {
    unpack_max_of_packed_bf16(local_amax_reg, local_amax);
    local_amax = WarpReduceMax<kThreadsPerWarp>(local_amax);
  ...
  if constexpr (kUpdateTransposeAmax) {
    unpack_max_of_packed_bf16(local_amax_t_reg, local_amax_t);
    local_amax_t = WarpReduceMax<kThreadsPerWarp>(local_amax_t);
    // broadcast the amax to all threads in a warp from the lane 0
    constexpr int lane_zero = 0;
...
void hadamard_transform_amax_sm100_plus(
    ...
    SimpleTensor& output_pre_rht_amax,    //
    SimpleTensor& output_identity_amax,   //
    SimpleTensor& output_transpose_amax,  //
    // TODO(Frank): Are we having random_sign_mask and random_sign_mask_t or RHT 16x16 matrix?
    uint16_t random_sign_mask,    //
    uint16_t random_sign_mask_t,  //
    int32_t hadamard_dimension,   //
    bool return_pre_rht_amax,     //
    bool return_identity_amax,    //
    bool return_transposed_amax,  //
    cudaStream_t stream) {
  ...
  NVTE_CHECK(input.shape.size() >= 2, "Input must be a 2D tensor.");
  NVTE_CHECK(hadamard_dimension == kHadamardDimension,
  ...
  NVTE_CHECK(row_length % hadamard_dimension == 0,
             "row_length must be divisible by hadamard_dimension.");

  NVTE_CHECK(num_rows % hadamard_dimension == 0,
  ...
  // TODO(Frank): Fix API usage.
  alignas(64) CUtensorMap tensor_map_input{};

  // TODO(Frank): This is not working. We will need to change this to
  // Swizzle 128B with atomicity 32
  ...
  dim3 grid(RoundUpDivide(row_length, kChunkBlockXSmall),
            RoundUpDivide(num_rows, kChunkBlockYSmall));
...
void hadamard_transform_sm90_plus(const SimpleTensor& input, SimpleTensor& output,
                                  ...
                                  bool return_identity, bool return_transposed,
                                  // TODO(Frank): Perhaps we expose support inverse hadamard?
                                  // bool inverse_hadamard,
                                  cudaStream_t stream) {
  NVTE_API_CALL(hadamard_transform_sm90_plus);

  bool inverse_hadamard = false;
  constexpr int kHadamardDimension = 16;
  ...
  dim3 grid(RoundUpDivide(row_length / hadamard_dimension, kThreadBlockX),
            RoundUpDivide(num_rows / hadamard_dimension, kThreadBlockY));
  ...
  TRANSFORMER_ENGINE_SWITCH_CONDITION(
      ...
      TRANSFORMER_ENGINE_SWITCH_CONDITION(
          ...
          kernel<<<grid, block, shmem_bytes, stream>>>(
              ...
              num_rows, row_length, nullptr, nullptr, inverse_hadamard);););
