...
class GetRecipes:
    ...
    @staticmethod
    def nvfp4_recipe_to_test(with_rht: bool):
        if with_rht:
...
def setup_environment_for_reference(with_rht: bool):
    if with_rht:
...
def check_nvfp4_module_versus_reference(
    ...
    setup_environment_for_reference(with_rht)
    reset_rng_states()
    ...
    with torch.no_grad():
        ...
        if hasattr(native_module, "layer_norm_weight") and hasattr(
            ref_module, "layer_norm_weight"
        ):
            ref_module.layer_norm_weight.copy_(native_module.layer_norm_weight)
        if hasattr(native_module, "layer_norm_bias") and hasattr(
            ref_module, "layer_norm_bias"
        ):
            ref_module.layer_norm_bias.copy_(native_module.layer_norm_bias)

    nvfp4_recipe = GetRecipes.nvfp4_recipe_to_test(with_rht)
    ...
    for step in range(num_steps):
        ...
        x_val = torch.normal(
            mean=0.0, std=1.0, size=x_shape, dtype=x_dtype, device=device
        )
        ...
        ref_outputs.append(
            {
                ...
                "input_grad": (
                    x_ref.grad.detach().clone() if x_ref.grad is not None else None
                ),
                "weight_grad": (
    ...
    for step in range(num_steps):
        ...
        torch.testing.assert_close(
            ...
            atol=1e-6,
            rtol=1e-6,
            msg=f"Input gradient mismatch at step {step}. Native: {native_out['input_grad']}, Ref: {ref_out['input_grad']}",
        )
        ...
        torch.testing.assert_close(
            ...
            atol=1e-6,
            rtol=1e-6,
            msg=f"Weight gradient mismatch at step {step}. Native: {native_out['weight_grad']}, Ref: {ref_out['weight_grad']}",
        )

        # Compare bias gradients
        if (
            bias
            and native_out["bias_grad"] is not None
            and ref_out["bias_grad"] is not None
        ):
            torch.testing.assert_close(
...
def check_nvfp4_layernorm_linear_versus_reference(
    ...
    with torch.no_grad():
        ...
        if hasattr(native_module, "layer_norm_bias") and hasattr(
            ...
            if (
                native_module.layer_norm_bias is not None
                and ref_module.layer_norm_bias is not None
            ):
                ref_module.layer_norm_bias.copy_(native_module.layer_norm_bias)
    ...
    for step in range(num_steps):
        ...
        cleanup_environment()
        with fp8_autocast(enabled=True, fp8_recipe=nvfp4_recipe):
            y_native, ln_out_native = native_module(
                x_native, is_first_microbatch=(step == 0)
            )
        y_native.backward(grad_output)
...
@pytest.mark.parametrize(
    "normalization", ["LayerNorm", "RMSNorm"], ids=["LayerNorm", "RMSNorm"]
)
@pytest.mark.parametrize("with_rht", [True, False], ids=["with_rht", "no_rht"])
